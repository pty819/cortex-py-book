# 第25章 竞品分析：Agent 记忆/知识图谱系统在复杂故障诊断场景的能力边界
...
## 5. MindMemOS（华为）：自演化记忆操作系统，Cortex-PY 三大自演化特性的原型

### 5.1 为什么单独列出

前四个竞品（Mem0 / Graphiti / agentmemory / OpenViking）都没有**记忆自演化**机制——它们要么靠人工触发 maintenance，要么靠 LLM 在写入时做一次性 ADD/UPDATE/DELETE。MindMemOS（华为）是唯一把"记忆如何根据反馈和离线巩固自我修正"作为一等公民设计的系统。

Cortex-PY 的三个自演化特性——**Feedback / Dreaming / Higher-Order**——直接借鉴自 MindMemOS，但在三个关键点上做了分叉。对诊断场景而言，理解这层借鉴关系很重要：它解释了 cortex-py 为什么这样设计，以及在哪些地方故意走了一条不同的路。

### 5.2 三大自演化特性的对应关系

| Cortex-PY 特性 | MindMemOS 原型 | 借鉴点 | cortex-py 的分叉 |
|----------------|----------------|--------|------------------|
...
MindMemOS 不是为故障诊断设计的——它的目标是通用 agent 的长期记忆演化。但它验证了"记忆必须能自演化"这个方向。cortex-py 在借鉴其架构骨架的同时，把三条特性都绑到了诊断场景的结构化约束上（谓词闭集、assertion_status、实体类型体系），使自演化不会生成违反本体约束的记忆。

---

## 6. 诊断场景的七个"不可妥协"的技术需求

综合以上分析，复杂机械故障诊断的知识图谱系统在技术上有七个**不可妥协**的需求。每个需求对应一个设计决策：
...
**谁做到了**：Cortex-PY（21 种实体类型 + 36 个标准谓词 + 四大类约束 + graph_eligible 准入规则）、Graphiti（可自定义 Pydantic 实体/边类型，但无约束强制执行机制）
...
### 需求 7：记忆必须能根据反馈自演化——而非只靠人工 maintenance 触发

```
为什么：
  诊断知识图谱不是一次写定的快照——它在持续使用中被修正。
  工程师反馈"这条因果链其实不对"、用户反馈"这个结论有用"，
  这些信号必须能即时影响后续检索的权重，
  而不是等到运维人员半夜跑一次 maintenance 才生效。

  如果记忆只能靠人工 maintenance 触发演化，
  它就会在两次 maintenance 之间反复犯同样的错——
  被排除的假设还会被召回，被确认的根因还会被降权。
```

**谁做到了**：Cortex-PY（Feedback / Dreaming / Higher-Order 三特性，借鉴 MindMemOS 架构并扩展正反馈通道与共享信号总线耦合）、MindMemOS（原型，负反馈 3 层 durable + 两阶段 Dreaming + 证据驱动高阶归纳）
...
|---------|:-----------:|:-------:|:-----------:|:-------------:|:--------------:|:------------:|
| **实体类型体系**（21种，含命名规范） | ✅✅ | ❌ | ⚠️ 自定义 Pydantic 但无约束 | ❌ | ❌ | ❌ |
...
| **轻量部署**（不需要额外图数据库） | ✅ PostgreSQL | ✅✅ SaaS/Redis | ❌ 需Neo4j | ✅ FS | ✅✅ SQLite | ⚠️ |
...
| **MindMemOS** | 不是诊断专用——无实体类型体系/谓词闭集/断言认知状态；无正反馈通道（只处理负反馈）；用 status='archived' 而非双时态软关闭；三特性解耦无共享信号总线 |
...
> - MindMemOS 是**自演化引擎**（记忆根据反馈和离线巩固自我修正）
