...
| **场景（+1）** | 全体干系人 | 验证：少数关键用例把四个视图缝合起来 | 脚本步骤 | 对象间与进程间交互 | 用例图、时序图 |
...
场景视图在 Kruchten 的模型里是**冗余**的（故称 +1），承担两个作用：在架构设计期**驱动发现**架构元素；在架构完成后**验证与说明**四个视图描述的是同一套系统。选场景的标准：最核心（系统存在的理由）、最高频、或风险最大。每个场景先给交互脚本，再给一张**四视图对照表**——这是缝合的关键；实现级细节回链对应章节。
...
**场景**：写入 / 召回 / 遗忘三条关键路径各配一张四视图对照表，验证五个投影说的是同一套系统。
```
