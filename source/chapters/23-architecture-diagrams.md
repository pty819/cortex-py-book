...
cortex-py 的功能需求围绕四条主线：**记忆读写**（长期记忆的写入与召回）、**知识治理**（浏览、手工增删改查、反馈、遗忘）、**根因诊断（RCA）**（Case 工作台与 Playbook 推理）、**后台管理**（功能开关、调参、审批、监控）。
...
| 用例 | 参与者 | 需求要点 |
|------|--------|---------|
| UC1 写入经验 | 知识使用者（①②③） | 幂等 WAL 入库；`?wait=` 同步语义 |
| UC2 知识预灌入 | 知识使用者（①②③） | 5 种导入器（jsonl / mem0 / zep / letta / openai）；长文档按标题分块 |
| UC3 召回记忆 | 知识使用者（①②③） | 6 通道混合检索 + 融合重排，返回分层证据包（StratifiedPack） |
| UC4 提问 · 流式回答 | 知识使用者（①②③） | 强制 think；SSE 分流 reasoning / answer；引用与 pack 可追溯 |
| UC5 知识全景浏览 | 知识使用者（①②③） | 分层预览 entities / facts / beliefs / understanding 与图谱可视化 |
| UC6 手工知识增删改查 | 知识使用者（①②③） | 单条 / 批量实体与事实的受控写；守卫一致、留审计 |
| UC7 反馈信号 | 知识使用者（①②③） | relevant / irrelevant / wrong / partial；即时影响召回排序 |
| UC8 GDPR 遗忘 | 知识使用者（①②③） | 两阶段 preview / execute；引用计数真删 |
| UC9 管理诊断 Case | 知识使用者（①②③） | open → investigating → resolved → closed 全生命周期；事件挂接 |
| UC10 Similar Case 检索 | 知识使用者（①②③） | 按设备 / 症状 / 根因特征检索同构历史案例 |
| UC11 Playbook 正向推理 | 知识使用者（①②③） | 确定性 DAG 遍历，给出下一步检查与结论建议 |
| UC12 功能开关与超参数调优 | 诊断工程师③ · 运维人员④ | 白名单热更新即时生效，无需重启 |
| UC13 后台管理 | 运维人员④ | 演化候选审批（Dreaming / Higher-Order）；任务队列监控 |
...
选出的架构关键场景：UC1（最核心——系统存在的理由）、UC4（最高频）、UC8（风险最大——不可逆操作），外加 UC7 / UC12 两个次级场景。
...
**场景**：用例总览（3 个直接用户 + 1 个后台角色 × 13 个用例，参与者泛化表达功能面等价）承载功能需求；写入 / 召回 / 遗忘三条关键路径各配四视图对照表，验证五个投影说的是同一套系统。
```
