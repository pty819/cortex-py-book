...
- **知识入库** —— 把经验与外部知识写进记忆（写入路径，详见中篇第 3–9 章）；
- **知识出库** —— 从记忆中取出知识，服务提问与诊断（读取路径，详见下篇第 14–17 章）；
- **知识管理** —— 对已入库知识的治理：浏览、手工增删改查、反馈、遗忘、Case 管理（第 7、10–13、21–22 章）；
- **软件运维控制** —— 功能开关、超参数调优与后台管理（第 20、24 章）。
...
3. **③ 诊断工程师 · 网页控制面板** —— 直接人机交互，使用知识库的全部功能：预览所有知识、知识预灌入、单条 / 批量知识手工增删改查等；
4. **④ 运维人员 · 后台管理** —— 治理与运维（功能开关、超参数调优、演化候选审批、任务队列监控）。

三个直接用户**功能面等价但途径各异**——能用的是同一批子系统，具体交互方式与协议不同（MCP 工具调用 / REST 请求 / 面板操作）。因此不设抽象的中间参与者，三个直接用户**各自直连**到所需的三个知识子系统（知识入库 / 知识出库 / 知识管理）。**软件运维控制只属于运维人员**：诊断工程师与 Agent 是知识库的使用者，不负责运维、开关与调参。
...
| 知识入库 | UC2 知识预灌入 | 直接用户（①②③） | 5 种导入器（jsonl / mem0 / zep / letta / openai）；长文档按标题分块 |
| 知识出库 | UC3 召回记忆 | 直接用户（①②③） | 6 通道混合检索 + 融合重排，返回分层证据包（StratifiedPack） |
| 知识出库 | UC4 提问 · 流式回答 | 直接用户（①②③） | 强制 think；SSE 分流 reasoning / answer；引用与 pack 可追溯 |
| 知识出库 | UC5 Similar Case 检索 | 直接用户（①②③） | 按设备 / 症状 / 根因特征检索同构历史案例 |
| 知识出库 | UC6 Playbook 正向推理 | 直接用户（①②③） | 确定性 DAG 遍历，给出下一步检查与结论建议 |
| 知识管理 | UC7 知识全景浏览 | 直接用户（①②③） | 分层预览 entities / facts / beliefs / understanding 与图谱可视化 |
| 知识管理 | UC8 手工知识增删改查 | 直接用户（①②③） | 单条 / 批量实体与事实的受控写；守卫一致、留审计 |
| 知识管理 | UC9 反馈信号 | 直接用户（①②③） | relevant / irrelevant / wrong / partial；即时影响召回排序 |
| 知识管理 | UC10 GDPR 遗忘 | 直接用户（①②③） | 两阶段 preview / execute；引用计数真删 |
| 知识管理 | UC11 管理诊断 Case | 直接用户（①②③） | open → investigating → resolved → closed 全生命周期；事件挂接 |
...
选出的架构关键场景：UC1（最核心——系统存在的理由）、UC4（最高频）、UC10（风险最大——不可逆操作），外加 UC9 / UC12 两个次级场景。
