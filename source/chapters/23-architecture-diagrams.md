...
1. **① Agent · MCP 通道** —— Claude Code / 远程 Agent 的程序化调用；
2. **② Agent · REST 通道** —— RESTful API 直连的程序化调用；
3. **③ 诊断工程师 · 网页控制面板** —— 直接人机交互。控制平面暴露**全功能面**：预览所有知识、知识预灌入、单条 / 批量知识手工增删改查、功能开关与超参数调节调优等；
4. **④ 运维人员 · 后台管理** —— 治理与运维（演化候选审批、任务队列监控）。

三个直接用户**共享同一功能面**——①②③能做的用例相同，差别只在接入方式。用 UML 的**参与者泛化**表达：三个直接用户是「知识使用者」角色的特化（泛化箭头由特化指向一般）。功能开关与超参数调优由诊断工程师（面板）与运维人员共同持有。
...
| 子系统 | 用例 | 参与者 | 需求要点 |
|--------|------|--------|---------|
| 知识入库 | UC1 写入经验 | 知识使用者（①②③） | 幂等 WAL 入库；`?wait=` 同步语义 |
| 知识入库 | UC2 知识预灌入 | 知识使用者（①②③） | 5 种导入器（jsonl / mem0 / zep / letta / openai）；长文档按标题分块 |
| 知识出库 | UC3 召回记忆 | 知识使用者（①②③） | 6 通道混合检索 + 融合重排，返回分层证据包（StratifiedPack） |
| 知识出库 | UC4 提问 · 流式回答 | 知识使用者（①②③） | 强制 think；SSE 分流 reasoning / answer；引用与 pack 可追溯 |
| 知识出库 | UC5 Similar Case 检索 | 知识使用者（①②③） | 按设备 / 症状 / 根因特征检索同构历史案例 |
| 知识出库 | UC6 Playbook 正向推理 | 知识使用者（①②③） | 确定性 DAG 遍历，给出下一步检查与结论建议 |
| 知识管理 | UC7 知识全景浏览 | 知识使用者（①②③） | 分层预览 entities / facts / beliefs / understanding 与图谱可视化 |
| 知识管理 | UC8 手工知识增删改查 | 知识使用者（①②③） | 单条 / 批量实体与事实的受控写；守卫一致、留审计 |
| 知识管理 | UC9 反馈信号 | 知识使用者（①②③） | relevant / irrelevant / wrong / partial；即时影响召回排序 |
| 知识管理 | UC10 GDPR 遗忘 | 知识使用者（①②③） | 两阶段 preview / execute；引用计数真删 |
| 知识管理 | UC11 管理诊断 Case | 知识使用者（①②③） | open → investigating → resolved → closed 全生命周期；事件挂接 |
| 软件运维控制 | UC12 功能开关与超参数调优 | 诊断工程师③ · 运维人员④ | 白名单热更新即时生效，无需重启 |
...
**场景**：用例总览（3 个直接用户 + 1 个后台角色 × 4 个功能子系统 × 13 个用例，参与者泛化表达功能面等价）承载功能需求；写入 / 召回 / 遗忘三条关键路径各配四视图对照表，验证五个投影说的是同一套系统。
```
