...
cortex-py 的功能划分为**四个功能子系统**，每个子系统是一组内聚的用例（功能需求），全部包含在用例边界框内：

- **知识入库** —— 把经验写进记忆，运行时写入路径（详见中篇第 3–9 章）；
- **知识出库** —— 从记忆中取出知识，服务提问与诊断（读取路径，详见下篇第 14–17 章）；
- **知识管理** —— 使用知识时的伴随治理：全景浏览、反馈、遗忘（第 10–13、21–22 章）；
- **软件运维控制** —— 知识库运营与系统运维：知识预灌入、手工增删改查、诊断 Case 管理、功能开关与调参、后台管理（第 7、20、24 章）。

```{admonition} 用例子系统 ≠ 开发视图的子包
:class: note
用例子系统是**功能分组**（面向需求），与开发视图（23.4）的五个代码子包（面向实现组织）不是同一维度——例如"知识出库"子系统在代码上横跨 `graph.retrieval` 与 `diagnostics` 两个包。
```

系统有**三个直接用户**（按接入方式区分）与**一个后台角色**：

1. **① Agent · MCP 通道** —— Claude Code / 远程 Agent 的程序化调用，走 MCP 协议；
2. **② Agent · REST 通道** —— RESTful API 直连的程序化调用，走 HTTP 协议；
3. **③ 诊断工程师 · 网页控制面板** —— 直接人机交互，使用知识库：检索提问、诊断推理、知识浏览与反馈；
4. **④ 运维人员 · 后台管理** —— 知识库的内容运维与系统运维：知识预灌入、单条 / 批量知识手工增删改查、诊断 Case 管理、功能开关、超参数调优、演化候选审批、任务队列监控。

三个直接用户**功能面等价但途径各异**——能用的是同一批子系统，具体交互方式与协议不同（MCP 工具调用 / REST 请求 / 面板操作）。因此不设抽象的中间参与者，三个直接用户**各自直连**到所需的三个知识子系统（知识入库 / 知识出库 / 知识管理）。**软件运维控制只属于运维人员**：知识预灌入、手工增删改查与诊断 Case 管理属于知识库的运营工作，连同开关、调参与后台管理一起，均不面向直接用户。
...
graph LR
    ...
    subgraph SYS["cortex-py（用例边界框）"]
        subgraph SUB_IN["知识入库"]
            UC1(("写入经验"))
        end
        subgraph SUB_OUT["知识出库"]
            UC2(("召回记忆"))
            UC3(("提问 · 流式回答"))
            UC4(("Similar Case 检索"))
            UC5(("Playbook 正向推理"))
        end
        subgraph SUB_MGMT["知识管理"]
            UC6(("知识全景浏览"))
            UC7(("反馈信号"))
            UC8(("GDPR 遗忘"))
        end
        subgraph SUB_OPS["软件运维控制"]
            UC9(("知识预灌入"))
            UC10(("手工知识增删改查"))
            UC11(("管理诊断 Case"))
            UC12(("功能开关与超参调优"))
            UC13(("后台管理<br/>演化审批 · 队列监控"))
        end
    end
...
| 子系统 | 用例 | 参与者 | 需求要点 |
|--------|------|--------|---------|
| 知识入库 | UC1 写入经验 | 直接用户（①②③） | 幂等 WAL 入库；`?wait=` 同步语义 |
| 知识出库 | UC2 召回记忆 | 直接用户（①②③） | 6 通道混合检索 + 融合重排，返回分层证据包（StratifiedPack） |
| 知识出库 | UC3 提问 · 流式回答 | 直接用户（①②③） | 强制 think；SSE 分流 reasoning / answer；引用与 pack 可追溯 |
| 知识出库 | UC4 Similar Case 检索 | 直接用户（①②③） | 按设备 / 症状 / 根因特征检索同构历史案例 |
| 知识出库 | UC5 Playbook 正向推理 | 直接用户（①②③） | 确定性 DAG 遍历，给出下一步检查与结论建议 |
| 知识管理 | UC6 知识全景浏览 | 直接用户（①②③） | 分层预览 entities / facts / beliefs / understanding 与图谱可视化 |
| 知识管理 | UC7 反馈信号 | 直接用户（①②③） | relevant / irrelevant / wrong / partial；即时影响召回排序 |
| 知识管理 | UC8 GDPR 遗忘 | 直接用户（①②③） | 两阶段 preview / execute；引用计数真删 |
| 软件运维控制 | UC9 知识预灌入 | 运维人员④ | 5 种导入器（jsonl / mem0 / zep / letta / openai）；长文档按标题分块 |
| 软件运维控制 | UC10 手工知识增删改查 | 运维人员④ | 单条 / 批量实体与事实的受控写；守卫一致、留审计 |
| 软件运维控制 | UC11 管理诊断 Case | 运维人员④ | open → investigating → resolved → closed 全生命周期；事件挂接 |
| 软件运维控制 | UC12 功能开关与超参数调优 | 运维人员④ | 白名单热更新即时生效，无需重启 |
| 软件运维控制 | UC13 后台管理 | 运维人员④ | 演化候选审批（Dreaming / Higher-Order）；任务队列监控 |
...
选出的架构关键场景：UC1（最核心——系统存在的理由）、UC3（最高频）、UC8（风险最大——不可逆操作），外加 UC7 / UC12 两个次级场景。
...
### 23.6.3 场景二：召回 + 流式回答（UC3 的实例 · 最高频）
...
### 23.6.4 场景三：GDPR 遗忘（UC8 的实例 · 风险最大）
...
**场景**：用例总览（3 个直接用户 + 1 个后台角色 × 4 个功能子系统 × 13 个用例；直接用户各自直连三个知识子系统，运维控制仅运维人员）承载功能需求；写入 / 召回 / 遗忘三条关键路径各配四视图对照表，验证五个投影说的是同一套系统。
```
